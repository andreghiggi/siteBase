...
if($_POST['cmd'] == "add")
{
    $str = "INSERT INTO blog_categorias (nome, status) VALUES ('$nome', '$status')";
    $rs  = mysql_query($str) or die(mysql_error());
    
    redireciona("blog_categorias.php?ind_msg=1");
}

if($_POST['cmd'] == "edit")
{  
    $str = "UPDATE blog_categorias SET nome = '$nome', status = '$status' WHERE codigo = '$codigo'";
    $rs  = mysql_query($str) or die(mysql_error());
    
    redireciona("blog_categorias.php?ind_msg=2");
}
...
{   
    ...
    if(!$numC)
    {
        $str = "DELETE FROM blog_categorias WHERE codigo = '$codigo'";
        $rs  = mysql_query($str) or die(mysql_error());

        redireciona("blog_categorias.php?ind_msg=3");
    }
    
    redireciona("blog_categorias.php?ind_msg=4");
}
...
elseif($_GET['ind_msg'] == 4)
    $msg = '<div class="alert warning">Não foi possível excluir a categoria!<br>Existem produtos associados a ela!</div>';

$str = "SELECT * FROM blog_categorias WHERE codigo = '$codigo'";
$rs  = mysql_query($str) or die(mysql_error());
...
<div class="g12">
    ...
    <?
    $str = "SELECT * FROM blog_categorias ORDER BY nome";
    $rs  = mysql_query($str) or die(mysql_error());
    ...
    <table class="datatable">
        ...
        ?>
            <tr <?=$class?>>
                ...
                <td class="c">
                    <a class="btn i_pencil icon small" title="editar" href="blog_categorias.php?ind=2&codigo=<?=$vet['codigo']?>" >editar</a>
                    <a class="btn i_trashcan icon small" title="excluir" href="blog_categorias.php?cmd=del&codigo=<?=$vet['codigo']?>" onclick="javascript: if(!confirm('Deseja realmente excluir este registro?')) { return false }">excluir</a>
                </td> 
            </tr>
        <?
        }
